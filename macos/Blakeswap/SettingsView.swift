    ...
    private var watchtowerSettings: some View {
        GroupBox("Watchtowers") {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Stepper(value: $draft.environments[index].rescueFeeBasisPoints, in: 1...1000) {
                    Text("Rescue fee: \(percentage(environment.rescueFeeBasisPoints))")
                }.accessibilityIdentifier("rescue-fee")
                Slider(value: Binding(get: { Double(environment.rescueFeeBasisPoints) }, set: { draft.environments[index].rescueFeeBasisPoints = Int64($0) }), in: 1...1000, step: 1)
                    .accessibilityLabel("Rescue fee")
                    .accessibilityValue(percentage(environment.rescueFeeBasisPoints))
