...
extension Blakeswap_V1_StrategyView: Identifiable { var id: String { config.id } }
...
struct StrategySideDraft {
    ...
    init(_ side: Blakeswap_V1_StrategySide? = nil) {
    ...
    func value() throws -> Blakeswap_V1_StrategySide {
        var s = Blakeswap_V1_StrategySide()
...
struct StrategyDraft: Identifiable {
    ...
    init(_ strategy: Blakeswap_V1_StrategyView? = nil) {
    ...
    func edit(context: TradeContext) throws -> Blakeswap_V1_StrategyEdit {
        let a = try common.edit(context: context).config
        var c = Blakeswap_V1_StrategyConfig()
        ...
        var q = Blakeswap_V1_StrategyEdit(); q.config = c; q.expectedRevision = common.revision; q.enabled = common.enabled; q.acknowledgeRestoredBudget = common.acknowledgeRestored
        return q
    }
}
...
final class StrategyModel: ObservableObject {
    @Published private(set) var strategies: [Blakeswap_V1_StrategyView] = []
    @Published private(set) var busy = false
    @Published private(set) var loaded = false
    @Published private(set) var error: String?
    @Published private(set) var review: Blakeswap_V1_StrategyReview?
    private var reviewedEdit: Blakeswap_V1_StrategyEdit?
    ...
    func load(current: () -> TradeContext) async {
        ...
        do {
            ...
            let result = try Blakeswap_V1_StrategyList(serializedBytes: data)
    ...
    func review(_ draft: StrategyDraft, current: () -> TradeContext) async {
        ...
        do {
            ...
            let r = try Blakeswap_V1_StrategyReview(serializedBytes: data)
            guard !Task.isCancelled, context.matches(current()) else { return }
    ...
    func stop(_ p: Blakeswap_V1_StrategyView, permanent: Bool, current: () -> TradeContext) async {
        guard !busy, context.matches(current()) else { return }
        busy = true; error = nil
        do {
            var q = Blakeswap_V1_StopStrategyRequest(); q.id = p.id; q.expectedWallet = context.profile; q.expectedNetwork = context.network; q.expectedRevision = p.revision; q.stop = permanent
