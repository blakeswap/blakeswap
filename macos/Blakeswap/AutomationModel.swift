...
extension Blakeswap_V1_AutomationView: Identifiable { var id: String { config.id } }
...
struct AutomationDraft: Identifiable {
    ...
    init(_ policy: Blakeswap_V1_AutomationView? = nil) {
    ...
    func edit(context: TradeContext) throws -> Blakeswap_V1_AutomationEdit {
        ...
        func rate(_ n: String, _ d: String) throws -> Blakeswap_V1_AutomationRate {
            var r = Blakeswap_V1_AutomationRate(); r.numerator = try number(n, "Rate numerator"); r.denominator = try number(d, "Rate denominator")
            guard r.numerator > 0, r.denominator > 0 else { throw RPCError.message("Rate fractions must be positive.") }
            return r
        }
        var c = Blakeswap_V1_AutomationConfig()
        ...
        var p = Blakeswap_V1_AutomationEdit(); p.config = c; p.expectedRevision = revision; p.enabled = enabled
...
final class AutomationModel: ObservableObject {
    @Published private(set) var policies: [Blakeswap_V1_AutomationView] = []
    @Published private(set) var busy = false
    @Published private(set) var loaded = false
    @Published private(set) var error: String?
    @Published private(set) var review: Blakeswap_V1_AutomationReview?
    private var reviewedEdit: Blakeswap_V1_AutomationEdit?
    ...
    func load(current: () -> TradeContext) async {
        ...
        do {
            var q = Blakeswap_V1_AutomationQuery(); q.expectedWallet = context.profile; q.expectedNetwork = context.network
            let data = try await call("automation.list", q.jsonUTF8Data())
            let page = try Blakeswap_V1_AutomationList(serializedBytes: data)
    ...
    func review(_ draft: AutomationDraft, current: () -> TradeContext) async {
        ...
        do {
            ...
            let result = try Blakeswap_V1_AutomationReview(serializedBytes: data)
            guard !Task.isCancelled, context.matches(current()) else { return }
    ...
    func save(current: () -> TradeContext) async -> Bool {
        ...
        do {
            ...
            let result = try Blakeswap_V1_AutomationView(serializedBytes: data)
    ...
    func disable(_ policy: Blakeswap_V1_AutomationView, cancelOpen: Bool, current: () -> TradeContext) async {
        guard !busy, context.matches(current()) else { return }
        busy = true; error = nil
        var q = Blakeswap_V1_DisableAutomationRequest(); q.id = policy.id; q.expectedWallet = context.profile; q.expectedNetwork = context.network
