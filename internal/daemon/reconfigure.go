...
func canChangeNetwork(s State) error {
	if s.Capacity != nil && (s.Capacity.Reactivating || len(s.Capacity.Invalidated) != 0) {
		return errors.New("archived obligations lost their canonical settlement evidence; keep monitoring until reactivation and positive reconciliation complete")
	}
