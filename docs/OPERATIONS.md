...
Use **Wallet → Export portable backup** after onboarding, or the chosen-password export during setup. Choose a new destination and a password of at least 16 bytes. The selected wallet's state on all three networks is included; optionally include all wallet profiles. The authenticated encrypted manifest includes receive indexes, pending sends and reservations, terms, preimages, signed fee variants, tower jobs/receipts, reliable messages, and activity lineage/receipt evidence. It excludes installation passwords and endpoint credentials. Keep the chosen archive password separately. A recovery phrase cannot reconstruct random swap preimages or every previously issued rescue signature.

Wallet status shows the last export and a reminder after meaningful recovery state changes. Export takes a consistent local snapshot, restarts background workers before encryption, and publishes the archive atomically without overwriting an existing file. Polling does not rewrite archives. A backup can succeed even if recording its freshness reminder fails; the result then explicitly says to keep the saved file and export again later.

Use **Wallet/Settings → Import backup** in an existing installation or **Restore wallet → Backup file** during setup. Inspect the encrypted contents, choose a wallet and a new profile name, then import. Multi-profile archives can be imported one profile at a time. Portable files use their chosen password; legacy `.db` copies use the original `vault.password` contents and have unknown snapshot dates. Both formats enter the same recovery gate. Existing profiles are preserved, duplicate identities are refused, and incomplete installation publication is resumed after restart. Do not replace a live `state.db` by copying an old snapshot into it.

**Recovery in progress** lists unresolved recorded obligations. Original offers and queued publications are quarantined. Imported swaps never resume new funding or make a first public revelation from private saved bytes. Already witnessed secrets permit target-chain claims, while a restored refund requires positive confirmed refund of the incoming leg plus current own-target and maturity evidence; an incoming claim permanently suppresses refunds even after reorg. The original signed variants, fee limits, payouts and bounties remain in force. Exact pending payment retries are retained. Standalone tower refunds without peer safety evidence remain blocked. A positively contradicted canonical checkpoint immediately withdraws permission to stop monitoring affected restored swaps, payments, and tower jobs. This hold survives restart and re-export even if an incomplete scan still shows their last confirmed outcome. Fresh positive settlement evidence clears the affected hold; a confirmed tower target can clear independently during an unrelated peer-chain outage.

Once recovery is ready, an expired, cancelled, filled, or refunded historical order can be reviewed and manually recreated under a new ID with current funds and terms. Its exact signed source remains quarantined; it cannot be cancelled, replaced, or republished from the archive. Unknown original creation times and publication status stay unknown.

**Ready to trade** requires current observations from both chains and positive confirmed resolution of known obligations. Durably final rejections/expiries with neither funding leg prepared remain inactive. A sole recorded funded leg can resolve through its positively confirmed refund; a final never-funded maker with a known peer leg also waits for that leg’s confirmed refund. An active snapshot with unknown own publication cannot use a peer refund as proof of nonpublication. Payment readiness uses freshly verified signed variants and canonical chain checkpoints, not saved confirmation counts; bounded scans retain progress only while that history stays valid. A synchronized restored wallet with no recorded obligations can become ready; arbitrary later state omitted by an old file remains outside that claim. Reorgs reopen the affected holds. Keep newer backups and the original installation available until recovery is complete. Snapshots before accepted terms/funding references or before a necessary secret can remain blocked; two restored counterparties may each wait for evidence that the other snapshot omitted. A fresh unrelated wallet is a separate option, never an implicit replacement for the restored identity.
...
| Activity lineage and receipt evidence | Remain in complete backup coverage. Shared transaction evidence still classifies active sibling outputs after another output moves to archive. |
