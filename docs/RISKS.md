...
Lightning, Taproot swaps and shared custodial liquidity remain excluded. Manual
whole or bounded partial orders use the upgraded protocol; each accepted slice
is an independent contract pair with its own costs and deadlines. See the
[partial-fill design and cutover](PARTIAL_FILLS.md). Focused independent code
review and actual-node tests do not constitute an external security audit.
...
Nostr distributes data; it is not a consensus orderbook or proof of reserves. Offers can be stale, duplicated, overcommitted, expired, or visible on only one relay. The honest local daemon durably reserves whole funding coins, including per-fill funding fees, for open parents, pending take requests, accepted children and saved sends. It serializes child allocation against the exact signed parent revision and refuses reuse of assigned inputs. Advertised remaining quantity is not a promise that enough disjoint confirmed inputs exist for simultaneous fills. Confirmed change reused by another child creates a funding dependency; the siblings then share that ancestor's reorg risk. These local checks do not constrain a remote maker running different software: it can advertise unbacked offers. Takers must still validate the actual contract, and capital can be temporarily locked if the counterparty disappears.

Relays can censor, reorder, omit, retain, or lose messages. Redundant storage and retries improve availability without guaranteeing delivery. Public order status changes cannot revoke a previously shared transaction. Cancellation withdraws only available remainder; accepted children keep their obligations. Parent availability changes use at most one signed update per wall-clock second, so a pending update or clock rollback can pause admission. A stale request requires new review. An unanswered take request can expire at its signed offer deadline only before acceptance or prepared funding. A maker child can return inventory only after a positively closed funding gate and durable future-funding refusal establish that it was never signed; backend absence and imported uncertainty cannot establish that condition. Late peer outcomes remain monitored. Funding that may have been published retains its obligation even if no source currently reports it.

The hard cutover rejects incompatible development messages and authenticated
active or archived state without migrating them. Rejection preserves files and
credentials; opening a separate development profile does not settle obligations
in the old one. Keep the matching prior application and recovery material for
any prior experimental contracts until they are resolved. Do not delete an old
profile merely to make the upgraded application start.

Mailbox synchronization uses bounded pages, durable per-relay/filter progress and overlapping historical resweeps. A saturated timestamp can still hide records that standard relay filters cannot enumerate; a short page or EOSE does not prove complete historical delivery. Encrypted compaction preserves recovery records but cannot recover data that never arrived. See [capacity and synchronization limits](OPERATIONS.md#encrypted-history-and-capacity). Dynamic relay discovery, Tor routing, peer scoring and public anti-Sybil admission remain unimplemented.
...
The wallet supports a narrow P2WPKH coin-selection path with bounded input counts and persisted funding fees, and P2WSH contract spends. Dust, insufficient confirmed balance, or an insufficient fee allowance may stop execution. Wallet sends support 1–50 manually selected confirmed, unlocked coins, an exact total fee, and review/confirmation. Sending selected coins to your own current receive address can consolidate them; there is no automatic consolidation service, external signer negotiation, or general PSBT importer. Saved sends reserve their inputs before broadcast and retain all signed variants after ambiguous failures. Chain-specific estimates and bounded change-only send replacements are available; cancellation and funding acceleration are unsupported. See [fee limits and recovery](FEES.md). Outgoing IDs, confirmations, and errors distinguish broadcasts from confirmed outcomes. A low-fee send can remain pending and block network switching until six confirmations. Balance cards expose total confirmed, unlocked confirmed (available), reserved confirmed, and awaiting-confirmation deposit funds. Whole reserved inputs are counted once even when obligations overlap. Observed unspent own contract principal is reported separately, with an explicit unavailable state when observations fail; it is not included in deposit funds. Available labels and preflight are snapshots, so the daemon rechecks actual outputs and reservations before spending. Active admission limits remain separate from archived history; compaction cannot erase a child obligation, return conserved inventory or grant new spending authority. Receive indexes remain bounded at 10,000 per chain. See [encrypted history and capacity](OPERATIONS.md#encrypted-history-and-capacity).
